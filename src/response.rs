...
impl Response {
	pub async fn validation_exist_sql_injection(
		...
		let set_payload = match payload {
			...
			_ => return Ok("Payload option not found".to_string()),
		};

		Self::response(&client, url, set_payload, payload == "8").await
	}
	...
	async fn response(
		...
		if response.status().is_success() {
			...
			if !show_body {
				match Self::contains_sql_injection(&body) {
					Ok(message) => Ok(message),
					Err(error) => Ok(error),
				}
			} else {
				match Self::extract_information_schema(&body) {
					Ok(message) => Ok(message),
					Err(error) => Ok(error),
				}
			}
	...
	fn extract_information_schema(html_content: &str) -> Result<String, String> {
		...
		let selector = Selector::parse("div, p").map_err(|_| "Failed to parse selector".to_string())?;

		let css_regex = Regex::new(r"\{.*?\}").map_err(|_| "Failed to compile regex".to_string())?;
